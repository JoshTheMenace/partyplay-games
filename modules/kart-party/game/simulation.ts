...
import { resolveGroundContact } from './race-contact';
...
export function createRace(options: RaceOptions): Race {
  ...
  const racers: Racer[] = Array.from({length:Math.max(DEFAULT_GRID_SIZE,players.length)},(_,i)=>{
    ...
    return {id:player?.id??`cpu-${i}`,name:player?.name??DRIVERS[driver%DRIVERS.length].name,driver,bot:!player,x:p.x,z:p.z,y:p.y,verticalSpeed:0,airborne:false,rampCooldown:0,heading:p.heading,speed:0,lateral:0,s,checkpoints:0,lap:1,rank:i+1,coins:0,drift:0,driftSide:0,boost:0,shield:0,stun:0,frost:0,oil:0,magnet:0,star:0,item:null,itemCooldown:0,finishTime:null,lastUse:false,offroad:false,connected:true,distance:0,wallTime:0,coinsTaken:[],contactCooldown:0,stats:{startRank:i+1,maxSpeed:0,itemsUsed:0,hitsDealt:0,hitsTaken:0,shieldsBlocked:0,coinsCollected:0,driftBoosts:0,collisions:0,airtime:0}};
  });
  return {track:options.track,speedClass:isSpeedClass(options.speedClass)?options.speedClass:100,phase:'countdown',time:0,countdown:3.5,laps:clamp(options.laps??3,1,5),racers,hazards:[],events:[],seed:options.seed??82731,serial:0,personalCameraIds:[],firstFinish:null,firstHumanFinish:null,difficulty:options.difficulty??'normal'};
}
...
export function botInput(race: Race,racer: Racer): Input {
  const track=TRACKS[race.track],pace=speedMultiplier(race.speedClass),lookahead=12+racer.speed/pace*.62,lane=Math.sin(racer.driver*2.1)*3.3;
...
function move(race: Race,racer: Racer,input: Input,dt: number) {
  ...
  racer.contactCooldown=Math.max(0,(racer.contactCooldown??0)-dt);
...
export function stepRace(race: Race,inputs: Record<string,Input>,dt=STEP) {
  ...
  for(let i=0;i<race.racers.length;i++) for(let j=i+1;j<race.racers.length;j++) {
    ...
    if(resolveLoopContact(race,a,b))continue;
    resolveGroundContact(race,a,b);
  }
