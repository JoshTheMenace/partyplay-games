...
import { activateItem, hit, selectItem, tickStatuses, updateHazards } from './items';
...
export function stepRace(race: Race,inputs: Record<string,Input>,dt=STEP) {
  ...
  for(let i=0;i<race.racers.length;i++) for(let j=i+1;j<race.racers.length;j++) {
    ...
    if(a.finishTime===null&&b.finishTime===null&&Math.abs(a.y-b.y)<2.5&&d<2.5&&d>.001) {if(a.star>0) hit(race,b);if(b.star>0) hit(race,a);const push=(2.5-d)/2;a.x-=dx/d*push;a.z-=dz/d*push;b.x+=dx/d*push;b.z+=dz/d*push;}
  }
