...
export class QualityGovernor {
  ...
  constructor(readonly tiers: number, options: QualityGovernorOptions = {}) {
    this.tierIndex = options.initialTier ?? 0;
    if (!Number.isInteger(this.tierIndex) || this.tierIndex < 0 || this.tierIndex >= tiers) throw new Error('Invalid initial quality tier.');
    ...
    this.upP95Ms = options.upP95Ms ?? 18;
