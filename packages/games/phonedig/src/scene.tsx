/* Personal cameras follow one digger; the shared map follows the whole crew.
 * Both use the sprite atlas and rebuild the same terrain from seed plus delta.
 * Viewport size stays in presentation and never changes the simulation. */
...
function paintField(field: HTMLCanvasElement, view: PublicView, t: Ground) {
  ...
  const pixels = document.createElement('canvas'); pixels.width = GW; pixels.height = GH;
  pixels.getContext('2d')!.putImageData(img, 0, 0);
  g.imageSmoothingEnabled = false; g.drawImage(pixels, 0, 0, field.width, field.height);
  for (let r = 0; r < GH; r++) for (let c = 0; c < view.activeGW; c++) if (t.dirt[idx(c, r)] === 1) {
    sprites.draw(g, `${theme.id}.dirt${1 + (c * 7 + r * 3) % 5}`, c * 8, r * 8, 8, 8);
    if (t.ore[idx(c, r)] > 0) { g.fillStyle = '#ffd24a'; g.fillRect(c * 8 + 3, r * 8 + 3, 2, 2); }
  }
}
...
function FieldScene(props: SceneViewProps<Settings, PublicView, PrivateView>) {
  ...
  useEffect(() => {
    ...
    const frame = (now: number) => {
      ...
      vfx.setBudget(motion.matches ? 1 : EFFECT_BUDGET[shownTier]);
      const t0 = perf.now();
      perf.frameStart(t0);
    ...
    const draw = () => {
      ...
      if (raw) {
        ...
        view.render(drawn, 1, null, motion.matches);
...
function SchematicScene(props: SceneViewProps<Settings, PublicView, PrivateView>) {
  ...
  useEffect(() => {
    ...
    field.width = GW * 8; field.height = GH * 8;
    ...
    const draw = (now: number) => {
      ...
      if (view) {
        ...
        for (const r of shown.rocks) {
          if (!sprites.draw(g, `${theme.id}.rock`, sx(r.x), sy(r.y), 2 * scale, 2 * scale)) box(r.x, r.y, 2, 2, '#8b8378');
          if (r.state === 'wobble') { g.strokeStyle = '#ffd24a'; g.lineWidth = 2; g.strokeRect(sx(r.x) - 2, sy(r.y) - 2, 2 * scale + 4, 2 * scale + 4); }
        }
        ...
        for (const m of shown.monsters) {
          ...
          if (!sprites.draw(g, `${m.kind}${m.variant ? `.${m.variant}` : ''}.walk.L0`, sx(m.x), sy(m.y), 2 * scale, 2 * scale)) box(m.x, m.y, 2, 2, tone);
          if (m.hunting) { g.strokeStyle = '#ff5748'; g.lineWidth = 2; g.beginPath(); g.arc(sx(m.x + 1), sy(m.y + 1), scale + 2, 0, Math.PI * 2); g.stroke(); }
          if (m.pump > 0) {
        ...
        for (const q of shown.players) {
          ...
          if (q.downed) {
            ...
            if (!sprites.draw(g, `player.${q.suit ?? 'cobalt'}.walk.D${(motion.matches ? 0 : Math.floor(now / 150) % 2)}`, sx(q.x), sy(q.y), 2 * scale, 2 * scale)) box(q.x, q.y, 2, 2, worn);
            g.fillStyle = '#fff6e5'; g.font = `bold ${Math.max(12, scale * .85)}px Nunito, sans-serif`; g.textAlign = 'center';
            g.fillText(String(shown.players.indexOf(q) + 1), sx(q.x + 1), sy(q.y) - 4);
          }
          if (q.id === p.playerId) {
            g.globalAlpha = 1;
