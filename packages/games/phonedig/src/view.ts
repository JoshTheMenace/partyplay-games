...
export function resize() {
  ...
  const vx = Math.round(((cssW - VIS_COLS * scale) / 2) * dpr) / dpr;

  L = {
    cssW, cssH, scale, vx, ox: vx, oy: 0, dpr, k,
    visRows: cssH / scale,
    visCols: Math.min(cssW / scale, VIS_COLS),
    top: L.top,
  };
...
export function render(state: RenderState, alpha: number, stick: StickView | null = null) {
  if (!ctx) return;
  const g = ctx;
