/** Copied from Ichi's music/synth.ts (helpers exported). A tiny offline synthesizer: instruments write into stereo buses; a mix adds sends, ducking and a limiter. */
...
export const len = (s: number) => new Float32Array(Math.max(1, Math.round(s * SR)));
/** Release gate: 1 until dur, then an exponential fade. */
export const gate = (s: number, dur: number, rel: number) => s < dur ? 1 : Math.exp(-(s - dur) / rel);
export const attack = (s: number, a: number) => s < a ? s / a : 1;
...
export class Mix {
  ...
  finish(o: { room?: number; damp?: number; reverbGain?: number; echoTime?: number; echoFb?: number; lofi?: number; target?: number } = {}) {
    ...
    // The loop is a circle: the box starts full of the loop's end, and a first lap warms the release follower.
    let g = 1, sum = 0;
    for (let k = loop - look; k < loop; k++) sum += ahead[k];
    for (let j = 0; j < loop * 2; j++) {
      const i = j % loop; sum += ahead[i] - ahead[(i - look + loop) % loop]; const box = sum / look;
      g = box < g ? box : box + (g - box) * rel; if (j >= loop) { out.l[i] *= gain * g; out.r[i] *= gain * g; }
    }
    return out;
  }
}
