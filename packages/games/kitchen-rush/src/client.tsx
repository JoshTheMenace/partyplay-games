...
import { choppable, KITCHENS, RECIPES, foodLabel, hasBelt, hasPower, itemLabel, neutral, recipeFor, stationLabel, unbakedPizza, type Input, type Item, type Settings, type Ticket, type View } from './model';
...
function Controller({ publicView: view, playerId, setInput, releaseInput, connected, serverNowMs }: Props) {
  ...
  const dash = (down: boolean) => { if (down) enqueue('dash'); };
  const handlers = useRef({ change, enqueue, use, dash }); handlers.current = { change, enqueue, use, dash };
...
export const client: GameClientModule<Input, never, Settings, View, null> = {
  settingsWide: true, LobbyView: CookLobby,
  PersonalView: Solo,
