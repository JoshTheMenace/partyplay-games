...
import { CHARACTERS, type CharacterId, choppable, CHOP_SECONDS, COOK_SECONDS, KITCHENS, RADIUS, REACH, SPEED, WASH_SECONDS, dimensions, hasBelt, hasGust, hasPower, layout, neutral, recipeFor, unbakedPizza, type Chef, type Food, type Input, type Item, type Settings, type Station, type View } from './model';
...
export const rules: GameRules<State, Input, never, Settings, View, null> = {
  parseLobbyChoice(raw) {
    ...
    const choice = raw as { character?: CharacterId };
    if (typeof raw !== 'object' || Array.isArray(raw) || Object.keys(raw).some(key => key !== 'character') || !CHARACTERS.some(item => item.id === choice.character)) throw Error('Choose one of the six cooks.');
    return { character: choice.character };
  },
