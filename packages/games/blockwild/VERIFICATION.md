# Chunk streaming verification — 13 September 2026

Implemented version-seven 4,096×4,096×48 worlds with deterministic signed chunks, deep diamonds, a bounded terrain cache, background solid/water meshing, nearby simulation, local maps and compatible saves. All code changes are inside Blockwild. See [implementation and limits](CHUNKED-WORLDS.md).

- Final immutable build: `blockwild-chunks-final`, index SHA256 `f412e85c53fce3e611b9f8e1bcab8b354a806f6ec22234b46c73e5ea72527ae5`, available at `http://localhost:4402/?game=blockwild`. Built in the isolated Blockwild checkout; unrelated game/platform edits were excluded. The earlier user preview on 4400 remains untouched. Intermediate chunk preview 4401 was stopped.
- 122 Blockwild tests pass; the full isolated suite passed 383 tests before the final text/map-size adjustment. Final TypeScript, scoped oxlint and diff whitespace checks pass. Tests exercise negative and positive boundaries, ten separated players, 4,000 blocks of cache traversal, deterministic regeneration and deep ore, distant mining/placing, stations/homes/crops, save roundtrips, a full edit journal with populated homesteads/inventories, stale mesh jobs and worker/geometry disposal.
- Browser build `blockwild-chunks-20260913` (index `26dc19041e820fed8b313ebe85ad7102603352e91d719b0ab5944382c1ae9f2d`), Chromium room TF5P3S: host plus ten emulated phones joined, readied and started a new version-seven world. A normal save import placed ten players in distinct regions, including coordinates ±1,800. All ten loaded successfully with one worker each. Player one flew from X=1000.5 to 1130.79 through actual keyboard controls, returned to X=1000.69, mined the fixture stone and placed dirt using the game controls. A downloaded save confirmed the removed stone and added dirt at their global keys.
- During the outbound/return traversal, the phone retained 49 rendered chunks and 18 cached collision chunks. The sampled final 600-frame windows had p95 about 16.8 ms on this Mac's headless Chromium; initial preparation had slower frames up to about 200 ms. This is desktop emulation, not a phone performance claim. The transport carried roughly 5.2 KiB per snapshot for this ten-player fixture; sparse edits were only updated when changed.
- Importing that downloaded world and reloading the phone preserved its position and 1 mined/1 built totals without browser errors. Importing a version-six save retained the old world and removed every chunk worker (ten workers to zero). Finish → results → replay → close passed. All ten phone contexts and the owned browser were closed.
- The final build repeated ten-player join/start and map/display checks. Maps at 320×568, 390×844, 667×375 and 844×390 have no page-width overflow; representative screenshots were inspected. The short-screen map now leaves coordinates and Close visible. Host views at 1280×720 and 1920×1080 retain the ten-player roster. The final room and all contexts were closed without browser errors. Evidence, scripts and exported fixtures are in `output/playwright/blockwild-chunks/`, with final layout evidence in its `final/` subdirectory.

Remaining limits: 48-block vertical height, 8,192 changed cells in new worlds, bounded existing animal/farm/storage capacities, and shared bounded edit/entity broadcasts rather than per-player network filtering. Unmodified exploration does not spend the edit budget. Physical Android/iOS touch, sustained frame rate, temperature and battery use remain untested. No commits or pushes were made for this change. No Prettier was used. The code-golf skill was unavailable; the manual simplification pass kept one chunk store, one worker and the existing save/transport format.

---

# Diamond depth verification — 13 September 2026

New terrain version 6 keeps generated diamond ore at Y=1–5 and at least ten blocks below the lowest terrain surface within four horizontal blocks. This closes the shallow coastline/riverbank case caused by checking absolute Y alone. Versions 1–5 retain their generators; placed blocks and loaded saves are not migrated.

- 110 Blockwild tests and 371 isolated integration tests pass; TypeScript, scoped oxlint and diff whitespace checks pass. Three depth tests cover seven seeds, deep ore remaining obtainable, exact version 1–5 terrain fingerprints, old/new save roundtrips and invalid future versions. The new-world and version-six load tests failed before the fix and passed afterward.
- Final build: `blockwild-depth-20260913`, index SHA256 `d92f9723817a1d2caf6a39359c002a4c74c5730cf65d848b961fd7bb2354bc53`, served at `http://localhost:4400/?game=blockwild`. Build and checks ran in the isolated Blockwild checkout; other games' ongoing work was excluded. Earlier preview ports and user rooms remain untouched.
- Playwright Chromium room KQ43YX: watching host plus 390×844 emulated touch phone. Real join → ready → new world reported version 6; download/reload retained version 6; loading the previous version-five save retained version 5. Finish → results → replay → room close passed without browser errors. The new-world frame was visually inspected. Owned room, phone context and named browser bw-depth were closed afterward. Evidence is in `output/playwright/blockwild-depth/` in the platform workspace.

No Prettier, commits or pushes were run for this follow-up. The code-golf skill was unavailable; the manual simplification pass retained one version-gated terrain filter and the existing save format instead of migrating worlds or duplicating the generator. No new UI or model assets were needed. This is not physical-device performance evidence and does not implement chunk streaming; see [the expandable-world assessment](CHUNKED-WORLDS.md).
