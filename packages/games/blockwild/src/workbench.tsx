...
import { BENCH, BERRIES, CHEST, FUEL, FURNACE, SAPLING, cropItems, isTool, paletteFor, recipesFor, stackLimit, type Action, type PrivateView, type Recipe, type View } from './model';
 ...
 return <Modal title={chest?'Chest':station?.kind===FURNACE?'Furnace':size===3?'Crafting Table':'Inventory'} onClose={onClose} wide><div className="bw-workbench">
  ...
  <div className="bw-bench" data-pane={pane}>
   <section className="bw-pane" data-pane="inventory" aria-label="Inventory">
    ...
    <div className="bw-inv-actions"><ArcadeButton size="sm" disabled={pending||(!creative&&!FOODS.some(id=>inv[id]))} onClick={()=>act({type:'eat'})}>Eat</ArcadeButton><ArcadeButton size="sm" tone="lime" disabled={pending||(!creative&&!cropItems.some(id=>inv[id]))} onClick={()=>act({type:'plant'})}>Plant crop</ArcadeButton><ArcadeButton size="sm" tone="lime" disabled={pending||(!creative&&(version<4?!inv[BERRIES]||!inv[2]:!inv[SAPLING]))} onClick={()=>act({type:'grow'})}>{version<4?'Grow tree':'Plant oak sapling'}</ArcadeButton></div>
    <p className="bw-hint">{version<4?'Grow tree costs 1 berry and 1 dirt. ':'A sapling needs clear grass or dirt with open space above; it becomes an oak a little later. '}Aim at the ground before opening this screen. Select a hoe and Till clear grass or dirt first. Then select seeds, a carrot or a potato and Plant on farmland. Harvest ripe crops and replant.</p>
   </section>
