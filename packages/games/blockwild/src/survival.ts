import { BED, BENCH, BERRIES, BREAD, CAMPFIRE, CHEST, COAL, FARMLAND, FURNACE, GRAIN, H, ITEMS, ROAST, SEEDS, W, coords, dayLength, isNight, type Action, type Homestead, type Recipe, type StationView } from './model';
...
export function startSmelting(s:State,p:Actor,recipe:Recipe){
  ...
  const fuel=(p.inventory[COAL]??0)>0?COAL:4,costs={...recipe.costs};costs[fuel]=(costs[fuel]??0)+1;
  ...
  s.furnaces[i]={item:recipe.item,count:recipe.count,readyAt:s.time+(recipe.seconds??10)};p.message=`Smelting ${recipe.name}. Aim at the furnace and Use to collect when ready.`;
}
...
export function homesteadAction(s:State,p:Actor,a:Action,setBlock:(s:State,i:number,b:number)=>boolean){
  if(a.type==='eat'){
    ...
    const food=[BREAD,ROAST,BERRIES].find(id=>(p.inventory[id]??0)>0);if(food===undefined)throw new Error('Forage leaves for berries, or grow grain and craft bread.');
    p.inventory[food]--;p.food=Math.min(10,p.food+(food===BREAD?6:food===ROAST?4:2));p.message=`Ate ${ITEMS[food]}. A full food meter gradually restores health.`;return;
  }
  ...
  if(a.type==='plant'){
    ...
    if(s.mode!=='creative'&&!(p.inventory[SEEDS]>0))throw new Error('Craft seeds from berries first.');
    ...
    s.farms.push({i,readyAt:s.time+(watered?90:180)});p.message=watered?'Planted beside water. Ready in 90 seconds.':'Planted. Ready in 3 minutes; nearby water helps crops grow faster.';return;
  }
  if(a.type==='deposit'||a.type==='withdraw'){
    ...
    const n=Math.min(from[a.item]??0,64,999-(to[a.item]??0));if(n<=0)throw new Error('No items to move, or the destination stack is full.');
    to[a.item]=(to[a.item]??0)+n;from[a.item]-=n;if(!from[a.item])delete from[a.item];if(Object.keys(chest).length)s.chests[i]=chest;else delete s.chests[i];p.message=`${a.type==='deposit'?'Stored':'Took'} ${n} ${ITEMS[a.item]}.`;return;
  }
  if(a.type!=='use')throw new Error('Unknown world interaction.');
  if(kind===FURNACE){const job=s.furnaces[i];if(!job)throw new Error('Open Craft nearby to choose a smelting recipe.');if(s.time<job.readyAt)throw new Error(`Smelting: ${Math.ceil(job.readyAt-s.time)} seconds left.`);if((p.inventory[job.item]??0)+job.count>999)throw new Error('Make inventory space before collecting.');p.inventory[job.item]=(p.inventory[job.item]??0)+job.count;delete s.furnaces[i];p.message=`Collected ${job.count} ${ITEMS[job.item]}.`;return;}
  if(kind===FARMLAND){const farm=s.farms.find(f=>f.i===i);if(!farm)throw new Error('Plant seeds here to grow grain.');if(s.time<farm.readyAt)throw new Error(`Growing: ${Math.ceil(farm.readyAt-s.time)} seconds left.`);if((p.inventory[GRAIN]??0)>997||(p.inventory[SEEDS]??0)>997)throw new Error('Make room for 2 grain and 2 seeds.');p.inventory[GRAIN]=(p.inventory[GRAIN]??0)+2;p.inventory[SEEDS]=(p.inventory[SEEDS]??0)+2;s.farms=s.farms.filter(f=>f!==farm);p.message='Harvested 2 grain and 2 seeds. Replant whenever you like.';return;}
  if(kind===BED){if(s.players.some(other=>other.id!==p.id&&other.connected&&other.sleeping&&other.home?.bed===i))throw new Error('Someone is resting in this bed. Use another bed.');const home=bedSpawn(s,i);if(!home)throw new Error('Leave a clear standing space beside the bed.');p.home={...home,bed:i};p.sleeping=isNight(s.time,s.terrainVersion);p.message=p.sleeping?'Resting. Night passes when every connected explorer rests. Move to wake up.':'Home set. You will return here after death.';return;}
...
export function sleepTick(s:State){for(const p of s.players)if(p.sleeping&&(!p.home||!bedSpawn(s,p.home.bed)||distance(p,p.home.bed)>5))p.sleeping=false;const awake=s.players.filter(p=>p.connected);if(awake.length&&awake.every(p=>p.sleeping)&&isNight(s.time,s.terrainVersion)){s.time=(Math.floor(s.time/dayLength(s.terrainVersion))+1)*dayLength(s.terrainVersion);s.creatures=[];for(const p of s.players){p.sleeping=false;p.message='Morning. Your world is waiting.';}}}

// Validate the entire save extension before mutating the live world.
export function readHomestead(raw:unknown,grid:Uint8Array):Homestead {
  ...
  const inventory=(v:unknown)=>!!v&&typeof v==='object'&&!Array.isArray(v)&&Object.keys(v).length<=16&&Object.entries(v).every(([k,n])=>/^(0|[1-9]\d*)$/.test(k)&&Number(k)<ITEMS.length&&Number.isInteger(n)&&n>0&&n<=999);
  ...
  return{chests:Object.fromEntries(Object.entries(h.chests).map(([k,v])=>[k,{...v}])),furnaces:Object.fromEntries(Object.entries(h.furnaces).map(([k,v])=>[k,{item:v.item,count:v.count,readyAt:v.readyAt}])),farms:h.farms.map(f=>({i:f.i,readyAt:f.readyAt}))};
}
