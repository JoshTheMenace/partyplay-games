...
export function homesteadAction(s:State,p:Actor,a:Action,setBlock:(s:State,i:number,b:number)=>boolean){
  if(a.type==='eat'){
    if(p.food>=10)throw new Error('Already feeling well.');
    const foods=s.terrainVersion>=4?[COOKED_BEEF,COOKED_PORK,COOKED_CHICKEN,COOKED_MUTTON,BAKED_POTATO,BREAD,APPLE,CARROT,POTATO,BEEF,PORK,CHICKEN,MUTTON,BERRIES,FLESH]:[BREAD,ROAST,BERRIES],food=foods.includes(p.selected)&&(p.inventory[p.selected]??0)>0?p.selected:foods.find(id=>(p.inventory[id]??0)>0);if(food===undefined)throw new Error('Gather apples from oak leaves, or grow wheat and craft bread.');
    p.inventory[food]--;p.food=Math.min(10,p.food+(s.terrainVersion>=4?FOOD_POINTS[food]??2:food===BREAD?6:food===ROAST?4:2));emitSound(s,'eat',p);p.message=`Ate ${ITEMS[food]}. A full food meter gradually restores health.`;return;
  }
  ...
  if(kind===FARMLAND&&s.terrainVersion>=4){if(!harvestCrop(s,p,i))throw new Error('Select seeds, a carrot or potato, then Plant.');return;}
...
export function readHomestead(raw:unknown,grid:Grid,version:TerrainVersion=3):Homestead {
  ...
  const seen=new Set<number>();for(const f of h.farms){if(!f||!validIndex(String(f.i),FARMLAND)||seen.has(f.i)||!Number.isFinite(f.readyAt)||f.readyAt<0||f.readyAt>1e9+360||(f.crop!==undefined&&!cropItems.includes(f.crop))||(f.plantedAt!==undefined&&(!Number.isFinite(f.plantedAt)||f.plantedAt<0||f.plantedAt>1e9)))throw new Error('Invalid farm.');seen.add(f.i);}
  ...
  return{fuelUntil:{...fuelUntil},chests:Object.fromEntries(Object.entries(h.chests).map(([k,v])=>[k,{...v}])),furnaces:Object.fromEntries(Object.entries(h.furnaces).map(([k,v])=>[k,{item:v.item,count:v.count,readyAt:v.readyAt}])),farms:h.farms.map(f=>({i:f.i,readyAt:f.readyAt,...(f.crop!==undefined?{crop:f.crop}:{}),...(f.plantedAt!==undefined?{plantedAt:f.plantedAt}:{})}))};
}
