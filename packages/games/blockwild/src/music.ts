...
export class BackgroundMusic{
 ...
 constructor(ctx:AudioContext,destination:AudioNode,private audio:HTMLAudioElement=new Audio()){
  audio.preload='none';audio.loop=false;
  this.source=ctx.createMediaElementSource(audio);this.gain=ctx.createGain();this.gain.gain.value=.18;
  this.source.connect(this.gain);this.gain.connect(destination);
  audio.onended=()=>this.next();audio.onerror=()=>{this.failed.add(this.index);this.next();};this.load();
 }
 ...
 dispose(){
  this.closed=true;this.audio.onended=this.audio.onerror=null;this.audio.pause();this.audio.removeAttribute('src');this.audio.load();this.source.disconnect();this.gain.disconnect();
 }
}
