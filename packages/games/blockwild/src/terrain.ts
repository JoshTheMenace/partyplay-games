...
export function terrain(seed:number,_version:TerrainVersion=2) {
  if(_version===3)return naturalTerrain(seed);
...
export function ray(grid: Uint8Array,x: number,y: number,z: number,yaw: number,pitch: number,reach=5) { const dx=-Math.sin(yaw)*Math.cos(pitch),dy=Math.sin(pitch),dz=-Math.cos(yaw)*Math.cos(pitch); let previous={x:Math.floor(x),y:Math.floor(y),z:Math.floor(z)}; for(let d=0;d<=reach;d+=.025) { const p={x:Math.floor(x+dx*d),y:Math.floor(y+dy*d),z:Math.floor(z+dz*d)}; if(solid(block(grid,p.x,p.y,p.z))) return { ...p, i:index(p.x,p.y,p.z), previous, distance:d }; previous=p; } return null; }
