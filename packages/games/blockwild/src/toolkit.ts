import { BONE_MEAL, BUCKET, WATER_BUCKET, SHEARS, WOOL, FARMLAND, SHORT_GRASS, H, W, coords, index, stackLimit, type Animal } from './model';
...
export function bucketUse(s:State,p:Actor,set:Edit){
  ...
  if(c.x<0||c.z<0||c.x>=W||c.z>=W||c.y<1||c.y>=H)throw new Error('Keep water inside the island.');
  if(empty?s.grid[i]!==6:s.grid[i]!==0)throw new Error(empty?'Aim directly at water.':'Water needs an empty block; it cannot replace plants or buildings.');
  if(!empty&&s.farms.some(f=>f.i===i-W*W))throw new Error('Pour beside the crops, not onto them.');
...
export function fertilize(s:State,p:Actor,i:number,set:Edit){
  ...
  }else if(s.grid[i]===1&&block(s.grid,c.x,c.y+1,c.z)===0){
...
export function spreadGrass(s:State,dt:number,set:Edit){
  ...
  for(const [i,b]of s.edits)if(b===2&&hash(i,Math.floor(s.time/10),s.seed)<.15){const c=coords(i);if(block(s.grid,c.x,c.y+1,c.z)!==0)continue;if([[1,0],[-1,0],[0,1],[0,-1]].some(([dx,dz])=>block(s.grid,c.x+dx!,c.y,c.z+dz!)===1))set(s,i,1);}
}
