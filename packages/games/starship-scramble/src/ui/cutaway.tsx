...
export type CutawayProps = { fit: CutawayFit; rooms: RoomView[]; crew: Crew[]; paint: string; faction: 'allied' | 'enemy'; timeMs: number; viewerId: string | null; captainColors: Map<string, string>; highlightRoomId?: string | null; selectedCrewId?: string | null; destroyed?: boolean };
...
export function CutawaySvg({ fit, rooms, crew, paint, faction, timeMs, viewerId, captainColors, highlightRoomId, selectedCrewId, destroyed }: CutawayProps) {
  ...
  const part = (p: Part, i: number) => {
    ...
    if (p.kind === 'barrel' && p.points && p.w) return <line key={i} x1={p.points[0][0]} y1={p.points[0][1]} x2={p.points[1][0]} y2={p.points[1][1]} stroke={ink} strokeWidth={p.w + 4} strokeLinecap="round"/>;
  ...
  return <g className={`ss-cut ss-cut-${faction}`} data-hull-x={Math.round(fit.hull.x)} data-hull-w={Math.round(fit.hull.w)}>
    <defs>
      ...
      <pattern id={id('unknown')} width="10" height="10" patternUnits="userSpaceOnUse"><circle cx="5" cy="5" r="1.4" fill="#a9b3e655"/></pattern>
    ...
    <g className="ss-cut-rooms">{fit.rooms.map(rect => { const room = rooms.find(r => r.id === rect.id)!, tier = effectiveTier(room, timeMs), cold = room.known && room.oxygen < 25, occupied = crew.some(c => c.roomId === rect.id), furniture = room.known && room.system ? FURNITURE[room.system] ?? [] : [];
      const tint = !room.known ? null : room.system === 'medical' || room.system === 'medical-support' ? '#3d9a6a' : room.system === 'shields' || room.system === 'teleporter' || room.system === 'shield-projector' || room.system === 'cloak' ? '#28c6e7' : room.system === 'weaponry' || room.system === 'point-defense' || room.system === 'boarding-defense' ? '#ffb347' : room.system === 'engines' ? '#ff8a5b' : room.system === 'piloting' || room.system === 'scanner' || room.system === 'hacking' ? '#78c9e8' : null;
      ...
      return <g key={rect.id} className={`ss-room ${highlightRoomId === rect.id ? 'ss-room-highlight' : ''}`} data-room={rect.id} data-occupied={occupied}>
        <rect x={rect.x + 1.5} y={rect.y + 1.5} width={rect.w - 3} height={rect.h - 3} rx={3} fill={room.known ? cold ? '#22384e' : `url(#${id('floor')})` : `url(#${id('unknown')})`}/>
        {room.known && <rect x={rect.x + 1.5} y={rect.y + 1.5} width={rect.w - 3} height={rect.h - 3} rx={3} fill={`url(#${id('deck')})`}/>}
        ...
        <rect x={rect.x + inset} y={rect.y + inset} width={rect.w - inset * 2} height={rect.h - inset * 2} rx={2} fill="none" stroke={room.locked ? '#ffd24a' : room.known ? `url(#${id('steel')})` : '#5a6a7f'} strokeOpacity={room.known ? 1 : .45} strokeWidth={room.locked ? wall + 1 : wall}/>
        {room.known && <rect x={rect.x + inset + wall / 2} y={rect.y + inset + wall / 2} width={rect.w - inset * 2 - wall} height={rect.h - inset * 2 - wall} rx={1.5} fill="none" stroke="#0b1220" strokeOpacity=".7" strokeWidth={1}/>}
        {room.known && [[rect.x + inset, rect.y + inset], [rect.x + rect.w - inset, rect.y + inset], [rect.x + inset, rect.y + rect.h - inset], [rect.x + rect.w - inset, rect.y + rect.h - inset]].map(([cx, cy], i) => <circle key={i} cx={cx} cy={cy} r={Math.max(1, wall * .28)} fill="#dfe6ea" opacity=".75"/>)}
        ...
        {room.known && !occupied && rect.w >= 66 && rect.h >= 52 && <text x={rect.x + 5} y={rect.y + rect.h - 6} className="ss-room-name" fill="#c9d2ff">{room.system ? roomTitle(room) : 'Bay'}</text>}
