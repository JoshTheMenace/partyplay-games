...
import { hulls, systems as SYSTEM_DEFS } from '../definitions/presentation/index';
...
function PreviewStage({ hullId, rooms, crew, paint, viewerId, captainColor }: { hullId: string; rooms: ReturnType<typeof roomsFromHull>; crew: Crew[]; paint: string; viewerId: string; captainColor: string }) {
  ...
  return <div ref={ref} className="ss-preview-stage" data-tile={fit.tile}><svg className="ss-interior-art" viewBox={`0 0 ${size.w} ${size.h}`} preserveAspectRatio="xMidYMid meet" role="img" aria-label={`Cutaway of the ${hullId} hull painted ${paint}`}>
    <CutawaySvg weaponIds={hulls.find(h => h.id === hullId)?.startingWeapons} fit={fit} rooms={rooms} crew={crew} paint={paint} faction="allied" timeMs={0} viewerId={viewerId} captainColors={new Map([[viewerId, captainColor]])}/>
  </svg></div>;
}
...
export function RouteMap({ beacons, currentId, votes, height, width: widthProp, labels = false, compact = false, myVote = null, onSelect }: RouteMapProps) {
  ...
  const varied = lanes > 2, offset = (b: Beacon) => varied ? Math.sin((b.column + 1) * 4.7 + b.lane * 2.3) : 0;
  const cx = (b: Beacon) => pad + b.column * ((width - pad * 2) / Math.max(1, columns - 1)) + offset(b) * 7, cy = (b: Beacon) => (varied ? 36 : 30) + b.lane * ((H - (varied ? 36 : 30) - bottom) / Math.max(1, lanes - 1)) + offset(b) * 6;
  ...
  const node = (b: Beacon, st: BeaconState) => { const x = cx(b), y = cy(b), r = st === 'current' ? 11 : st === 'reachable' ? 9 : st === 'visited' ? 7 : 5, kindFill = b.kind === 'exit' ? '#78d955' : b.kind === 'store' ? '#28c6e7' : b.kind === 'combat' ? '#ff5748' : '#a9b3e6';
    ...
    if (st === 'locked') return <circle cx={x} cy={y} r={r + 1} fill={kindFill} opacity={.7} stroke="#05071a" strokeWidth={1.5}/>;
  ...
  return <div className={`ss-route-map ${onSelect ? 'ss-route-map-live' : ''}`} style={{ aspectRatio: `${width} / ${H}` }}>
    <svg viewBox={`0 0 ${width} ${H}`} role="img" aria-label={`Route map: ${beacons.length} beacons, ${reachable.size} within jump range${current ? `, fleet at ${beaconCode(current)}` : ', at departure'}`}>
      ...
      {beacons.flatMap(b => b.next.map(id => { const n = beacons.find(x => x.id === id); if (!n) return null; const live = b.id === currentId, shortcut = n.column > b.column + 1; return <line key={b.id + id} x1={cx(b)} y1={cy(b)} x2={cx(n)} y2={cy(n)} stroke={live ? '#ffd24a' : shortcut ? '#b58aff' : '#fff6e5'} strokeOpacity={live ? .95 : shortcut ? .55 : .25} strokeWidth={live ? 3 : 1.5} strokeDasharray={shortcut ? '3 5' : undefined}/>; }))}
      {beacons.map(b => { const st = state(b), x = cx(b), y = cy(b), r = st === 'current' ? 11 : st === 'reachable' ? 9 : st === 'visited' ? 7 : 5, voters = votes.filter(v => v.vote === b.id); return <g key={b.id} data-beacon={b.id} data-code={beaconCode(b)} data-state={st}>{node(b, st)}
        ...
        {labels && !varied && (st === 'reachable' || st === 'current') && <text x={x} y={y + r + 14} textAnchor="middle" className="ss-route-label" fill="#fff6e5">{b.label}</text>}
        ...
        {compact && <text x={x} y={y + r + 12} textAnchor="middle" className="ss-route-kind" fill="#a9b3e6">{st === 'current' ? 'HERE' : st === 'visited' ? 'SEEN' : myVote === b.id ? 'VOTED' : b.kind === 'store' ? 'SHOP' : b.kind === 'combat' ? 'HOSTILE' : b.kind === 'exit' ? 'EXIT' : 'SIGNAL'}</text>}
