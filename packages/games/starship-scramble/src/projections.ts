...
export function interiorView(state: State, ship: Ship, captain: Captain, defs: Definitions): InteriorView {
  ...
  const visible = (roomId: string) => own || scanned || state.simulation.crew.some(crew => crew.currentShipId === ship.id && crew.roomId === roomId && crew.ownerCaptainId === captain.id && crew.status === 'alive');
