...
export type CombatEffect = { id: string; kind: 'shot' | 'impact' | 'teleport' | 'repair' | 'destroyed' | 'shield' | 'warning'; sourceShipId: string; targetShipId: string; text: string; atMs: number; weaponId?: string; roomId?: string; mountIndex?: number; flightMs?: number; delayMs?: number };
...
export type ShipSummary = Pick<Ship, 'id' | 'ownerCaptainId' | 'faction' | 'hullId' | 'name' | 'color' | 'formation' | 'hull' | 'maxHull' | 'status' | 'shield'> & { weaponIds: string[]; alerts: string[]; rooms: { id: string; name: string; system: SystemId | null }[]; crewCount: number; targetShipId: string | null; escapeAtMs: number | null };
