...
for (const allies of [1, 2, 3, 4]) for (const enemies of [0, 1, 2, 3, 4, 5, 6]) void test(`formation ${allies} allies vs ${enemies} enemies stays inside the stage without overlap`, () => {
  ...
  assert.ok(allySlots.every(s => s.x + s.w <= 420) && enemySlots.every(s => s.x >= 700), 'allies left, enemies right, firing lane between');
});
...
void test('ally slots hold 116px cards at 1280x620 for four captains and enemy cards keep 150px rows for six', () => {
  const slots = formation(fleet(4, 6));
  assert.ok(slots.filter(s => s.faction === 'allied').every(s => s.h === 116 && s.w === 360));
  assert.ok(slots.filter(s => s.faction === 'enemy').every(s => s.h === 150 && s.w === 250));
});
