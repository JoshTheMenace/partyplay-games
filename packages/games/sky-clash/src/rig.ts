...
import { STAGE, type Fighter, type FighterKind } from './model';
import { applyFlat, captureRig, emptyFlat, mixFlat, targetPose, type ActorClock, type Flat, type Rig } from './animation';
...
export class Actor {
  ...
  seat: Mesh; halo: Mesh; shield: Mesh; reflector: Mesh; disc: Mesh; shadow: Mesh; cue: Mesh; streak: Mesh; fire: Mesh; charge: Mesh;
  constructor(readonly kind: FighterKind, readonly color: string, source: Object3D, cache: Cache) {
    ...
    this.seat = flat(new Mesh(geo(cache, 'seat', () => new RingGeometry(.3, .4, 32)), basic(.85)), .012); this.group.add(this.seat);
    this.halo = flat(new Mesh(geo(cache, 'halo', () => new RingGeometry(.48, .6, 40)), basic(.8)), .02); this.group.add(this.halo);
    this.disc = flat(new Mesh(geo(cache, 'disc', () => new CircleGeometry(.8, 28)), basic(.55)), -.03); this.group.add(this.disc);
    this.charge = flat(new Mesh(geo(cache, 'charge', () => new RingGeometry(.5, .75, 40)), basic(.7, '#ffd24a')), .015); this.group.add(this.charge);
    this.shield = new Mesh(geo(cache, 'bubble', () => new SphereGeometry(1, 20, 14)), basic(.32)); this.shield.position.y = STAGE.fighterHeight * .52; this.group.add(this.shield);
    this.reflector = new Mesh(geo(cache, 'reflector', () => new CylinderGeometry(1.15, 1.15, .08, 6)), basic(.45, '#6fd8ff')); this.reflector.rotation.x = Math.PI / 2; this.reflector.position.y = STAGE.fighterHeight * .5; this.group.add(this.reflector);
    this.fire = new Mesh(geo(cache, 'fire', () => new SphereGeometry(1.15, 18, 12)), basic(.5, '#ff8a3d')); this.fire.position.y = STAGE.fighterHeight * .5; this.group.add(this.fire);
    this.streak = new Mesh(geo(cache, 'streak', () => new PlaneGeometry(2.2, .9)), basic(.45, kind === 'falco' ? '#7fb6ff' : '#ffb347')); this.streak.position.set(-1.3, STAGE.fighterHeight * .5, 0); this.turn.add(this.streak);
    ...
    for (const m of [this.halo, this.disc, this.charge, this.shield, this.reflector, this.fire, this.streak, this.cue]) m.visible = false;
  }
  ...
  update(f: Fighter, x: number, y: number, seconds: number, dt: number, reduced: boolean, phase: string) {
    ...
    const k = 1 - Math.exp(-dt * (striking ? 30 : f.mode === 'hurt' ? 26 : 15)); mixFlat(this.current, this.current, target, k); applyFlat(this.rig, this.current);
    ...
    const shieldAmount = Math.max(.15, f.shield / 100);
    this.shield.visible = f.mode === 'shield'; this.shield.scale.setScalar(.7 + shieldAmount * .55 + (reduced ? 0 : Math.sin(seconds * 9) * .015));
    ...
    this.reflector.visible = f.mode === 'attack' && f.move === 'reflect'; this.reflector.rotation.z = reduced ? 0 : seconds * 2; this.reflector.scale.setScalar(1 + (reduced ? 0 : Math.sin(seconds * 10) * .04));
    const rising = f.mode === 'attack' && f.move === 'rise'; this.fire.visible = rising; this.fire.scale.setScalar(.7 + (reduced ? 0 : Math.sin(seconds * 25) * .08) + Math.min(.5, Math.hypot(f.vx, f.vy) / 20)); (this.fire.material as MeshBasicMaterial).opacity = rising && (Math.abs(f.vx) > .5 || Math.abs(f.vy) > .5) ? .55 : .3;
    this.streak.visible = f.mode === 'attack' && f.move === 'dash' && f.moveFrame > 2; this.streak.scale.x = Math.min(2, .6 + Math.abs(f.vx) / 8);
    const landing = STAGE.platforms.filter(p => x >= p.left - .1 && x <= p.right + .1 && p.y <= y + .05).sort((a, b) => b.y - a.y)[0];
