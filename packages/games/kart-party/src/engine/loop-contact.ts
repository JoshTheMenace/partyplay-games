...
export function resolveLoopContact(race:Race,a:Racer,b:Racer){
  ...
  if(closing>0){const impulse=closing*.6;a.speed=Math.max(0,a.speed-impulse*direction);b.speed=Math.max(0,b.speed+impulse*direction);recordContact(race,a,closing,'kart');recordContact(race,b,closing,'kart');}
