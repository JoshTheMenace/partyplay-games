import { loadGarage } from './model-assets';
import { createCourseFeatures } from './course-feature-visuals';
import { COURSE_DESIGNS } from './course-designs';
import { racerRoute, routeGroundHeight } from './course-interactions';
import { routeNearest, routeSample } from './course-routes';
import { createRouteVisuals } from './route-visuals';
import { createCanyonDetails } from './courses/canyon-visuals';
import { createMidnightDetails } from './courses/midnight-visuals';
import { createRainbowDetails } from './courses/rainbow-visuals';
import { hazardPosition } from './items';
...
import { contactRecoil } from './contact-feedback';
...
import { angleDelta, bankAt, nearest, rampAt, roadHeight, sample, surfaceFrame, magneticAt } from './tracks';
...
export function createRenderer(container:HTMLElement,onError:(message:string)=>void) {
  ...
  const chase=cameras.map(()=>createChaseCamera()),motion=window.matchMedia('(prefers-reduced-motion: reduce)');
  const wasMagnetic=cameras.map(()=>false);
  let world:World|null=null,currentTrack:TrackId|null=null,quality='high',width=0,height=0,disposed=false,frameCount=0;
  const assetAbort=new AbortController();let garage:T.Group|null=null;
  void loadGarage(assetAbort.signal).then(model=>{if(disposed)disposeObject(model);else garage=model;}).catch(error=>{if(!disposed)onError(String(error));});
  let courseFeatures:ReturnType<typeof createCourseFeatures>|null=null,routeVisuals:ReturnType<typeof createRouteVisuals>|null=null;
  ...
  function changeTrack(id:TrackId) {
    ...
    courseFeatures=createCourseFeatures(world.track,garage!);world.group.add(courseFeatures.group);
    routeVisuals=createRouteVisuals(world.track,COURSE_DESIGNS[id]);world.group.add(routeVisuals.group);
    const details=id==='canyon'?createCanyonDetails:id==='midnight'?createMidnightDetails:id==='rainbow'?createRainbowDetails:null;if(details)world.group.add(details(world.track));
    ...
    for(const camera of chase)camera.reset();
  }
  function updateKart(kart:Kart,r:Racer,dt:number,interpolated=false) {
    const route=racerRoute(world!.track,r),p=route?routeSample(world!.track,route,r.s):sample(world!.track,r.s),kartHeight=r.y??p.y,distance=kart.group.position.distanceTo(new T.Vector3(r.x,kartHeight,r.z));
    ...
    const ramp=route?undefined:rampAt(world!.track,r.s,nearest(world!.track,r.x,r.z).offset);
    const height=(s:number)=>route?routeSample(world!.track,route,s).y:sample(world!.track,s).y;
    const slope=(ramp?1.2*ramp.height/ramp.length:0)+(height(r.s+2/world!.track.length)-height(r.s-2/world!.track.length))/4;
    ...
    const recoil=motion.matches?0:contactRecoil(r,raceTime);
  ...
  function render(frame:RenderFrame,dt:number,time:number) {
    if(disposed||!garage||!width||!height)return false;
    changeTrack(frame.race?.track??frame.track);frameCount++;raceTime=frame.race?.time??time;
    world!.update?.(time);courseFeatures?.update(raceTime);routeVisuals?.update(raceTime);
    ...
    for(const r of racers){let kart=karts.get(r.id);if(!kart||kart.driver!==r.driver||kart.kartId!==(r.kart??'standard')){if(kart){scene.remove(kart.group);disposeObject(kart.group);}kart=createKart(r.driver,garage,r.kart);karts.set(r.id,kart);scene.add(kart.group);kart.group.position.set(r.x,sample(world!.track,r.s).y,r.z);kart.group.rotation.y=r.heading;}kart.group.visible=true;updateKart(kart,r,dt,frame.interpolated);}
    ...
    for(let i=0;i<count;i++) {
      ...
      const pose=chase[i].sample(r,dt,!!race,motion.matches,frame.snappedIds?.includes(r.id)||wasMagnetic[i]!==!!magnetic);wasMagnetic[i]=!!magnetic;
      const p=sample(world!.track,r.s),kartHeight=r.y??p.y,angle=pose.heading;
      ...
      target.y=Math.max(target.y,cameraFloor(target.x,target.z,r.routeId));
      if(pose.cut)camera.position.copy(target);else camera.position.lerp(target,1-Math.exp(-dt*(race?7:2)));
      camera.position.y=Math.max(camera.position.y,cameraFloor(camera.position.x,camera.position.z,r.routeId));
      ...
      if(race)for(const t of [.2,.4,.6,.8]) {
        const floor=cameraFloor(T.MathUtils.lerp(camera.position.x,r.x,t),T.MathUtils.lerp(camera.position.z,r.z,t),r.routeId)-2;
      ...
      camera.fov=pose.fov;camera.far=world!.track.id==='rainbow'?6000:2500;camera.aspect=w/h;camera.updateProjectionMatrix();
  ...
  function cameraFloor(x:number,z:number,routeId?:string){
    const route=racerRoute(world!.track,{routeId}),p=route?routeNearest(world!.track,route,x,z):nearest(world!.track,x,z);
    const road=route?routeSample(world!.track,route,p.s,p.offset).y:roadHeight(world!.track,p.s,p.offset);
    return Math.max(routeGroundHeight(world!.track,x,z)+3,p.distance<(route?.width??world!.track.width)/2+3?road+3:-Infinity);
  }
  return {render,dispose(){disposed=true;assetAbort.abort();if(garage)disposeObject(garage);observer.disconnect();renderer.domElement.removeEventListener('webglcontextlost',contextLost);disposeObject(scene);sun.shadow.dispose();renderer.dispose();renderer.forceContextLoss();renderer.domElement.remove();}};
}
