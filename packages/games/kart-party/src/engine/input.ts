...
export function humanInput(touch:Input,keys:ReadonlySet<string>,autoAccelerate:boolean,active:boolean,pendingUse=false):Input {
  ...
  return {steer:steer===0?0:-steer,throttle:active&&(autoAccelerate||touch.throttle||keys.has('arrowup')||keys.has('w')),brake:touch.brake||keys.has('arrowdown')||keys.has('s'),drift:touch.drift||keys.has('shift')||keys.has(' '),use:touch.use||pendingUse||keys.has('e')||keys.has('enter')||keys.has('x')||keys.has('control')};
}
