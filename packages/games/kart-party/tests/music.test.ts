...
import { MUSIC, selectMusic, selectPlaylist, StreamedMusic, type MusicTrack } from '../src/engine/music';
...
void test('all four courses select deterministic playlists for all three speed classes, with faster primary tracks at higher cc',()=>{
  ...
  for(const song of Object.values(MUSIC)){assert.ok(existsSync(new URL('../public' + song.path.replace('/games/kart-party', ''), import.meta.url)));assert.ok(song.energy>0&&song.energy<=1);}
});
