...
import type { GameClient } from '../src/engine/client-types';
import { humanInput } from '../src/engine/input';
import * as simulation from '../src/engine/simulation';
import { NEUTRAL, type RoomView } from '../src/engine/types';
...
const source=ts.transpileModule(readFileSync(new URL('../src/engine/useGame.ts',import.meta.url),'utf8'),{compilerOptions:{module:ts.ModuleKind.CommonJS,target:ts.ScriptTarget.ES2022}}).outputText;
const requireGame=createRequire(new URL('../src/engine/useGame.ts',import.meta.url));
