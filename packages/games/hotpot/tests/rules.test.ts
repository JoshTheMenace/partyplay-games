...
import { delays, detectSets, rules, IDLE_MS } from '../src/server';
...
test('bots draw after a pause, discard after another, and cover an idle human after a full minute', () => {
  ...
  until(state, 0);
  const idleAt = state.botAt;
  state.hands[0] = [...SAFE];
  assert.ok(Number.isFinite(idleAt));
  rules.tick(state, new Map(), 0, idleAt - 1);
  assert.equal(state.current, 0); assert.equal(state.phase, 'draw');
  rules.tick(state, new Map(), 0, idleAt);
  assert.equal(state.phase, 'discard');
  rules.tick(state, new Map(), 0, idleAt + delays.discard);
  assert.equal(state.current, 1);
  assert.match(state.log.map(entry => entry.text).join(' '), /idle/);
});
...
test('a disconnected player is covered by a bot and takes over again on return', () => {
  ...
  assert.equal(state.botAt, 1200 + IDLE_MS);
  rules.tick(state, new Map(), 0, 1299);
  assert.equal(state.phase, 'discard');
