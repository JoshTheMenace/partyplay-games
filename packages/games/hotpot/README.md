...
Bots draw from the deck after 1 s, check for a win and discard at random 0.8 s later. They never take discards.
...
- Views: `DisplayView` is the shared table with every hand face down. `ControllerView` is the portrait phone layout. A host playing on this device gets the table with their hand at the bottom on wide screens and the phone layout on narrow ones. `ResultsView` reveals every hand and highlights the winning sets.
