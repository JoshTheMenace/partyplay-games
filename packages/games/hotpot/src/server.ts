import { chooseDraw, chooseDiscard } from './bot';
...
function setPhase(state: State, phase: Exclude<Phase, 'won'>, now: number) {
  ...
  state.botAt = now + (automated(state) ? delays[phase] : IDLE_MS);
}
...
export const rules: GameRules<State, null, Action, Settings, PublicView, PrivateView> = {
  ...
  tick(state, _inputs, _dt, now) {
    ...
    for (let step = 0; step < SEATS * 2 && state.phase !== 'won' && now >= state.botAt; step++) {
      const at = state.botAt, idle = !automated(state);
      if (idle) log(state, `${state.seats[state.current].name} is idle — a bot is covering this turn`);
      if (state.phase === 'draw') draw(state, chooseDraw(state.hands[state.current], state.piles, state.current), at);
      else discard(state, chooseDiscard([...state.hands[state.current], state.drawn!], random(state)), at);
      if (idle && state.phase === 'discard') state.botAt = at + delays.discard;
    }
  },
  onPresenceChange(state, playerId, connected, now) {
    ...
    if (seat === state.current) state.botAt = now + (connected ? IDLE_MS : delays[state.phase]);
  },
  publicView(state) {
    return {
