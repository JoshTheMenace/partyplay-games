...
function Card({ id, size = 'md', onClick, glow, pos, label, style }: { id: number | null; size?: 'xs' | 'sm' | 'md' | 'lg'; onClick?(): void; glow?: boolean; pos?: string; label?: string; style?: CSSProperties }) {
  ...
  return onClick ? <button type="button" {...props} onClick={onClick}>{image}</button> : <span role="img" {...props}>{image}</span>;
}
function useMove(props: Props) {
  ...
  async function send(action: Action) {
    ...
    try {
      ...
      if (!result.accepted) setError(result.reason || 'That move was not accepted.');
    } catch { setError('Could not send. Reconnect and try again.'); }
    finally { setPending(false); }
  }
  const mine = !!own && own.seat === view.current && !pending;
  return {
    error, own,
...
function Table(props: Props) {
  const view = props.publicView, { own, draw, discard, error } = useMove(props);
  const me = own?.seat ?? 0;
  return <main className="hotpot hotpot-table">
    <header className="hotpot-hud"><span className="hotpot-pill">Round {view.round}</span><p className="hotpot-status" aria-live="polite">{status(view, own)}</p><Log view={view} count={4} /></header>
...
function Phone(props: Props) {
  ...
  return <main className="hotpot hotpot-phone">
    <header className="hotpot-phone-head"><span className="hotpot-pill">Round {view.round}</span><p className="hotpot-status" aria-live="polite">{status(view, own)}</p></header>
...
function SettingsView() {
  return <Panel className="hotpot"><p>Seats up to four. Bots fill every empty seat and cover for anyone who disconnects until they return.</p></Panel>;
}
function InstructionsView() {
  return <Panel className="hotpot hotpot-instructions">
    ...
    <p>A set is one of each ingredient in a color, like crab claw, fish and shrimp, or three copies of the same card. The deck never runs out.</p>
  </Panel>;
}
