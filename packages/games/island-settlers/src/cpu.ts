/** Expansion decisions use the same legal targets and prompts as a human controller. */
import { chooseAction } from './cpu-base';
import { COSTS, RESOURCES, emptyHand, type Action, type PrivateView, type PublicView } from './model';
import { COMMODITIES, type ExpansionCommand, type Good } from './expansion-model';
...
export function chooseExpansionAction(pub: PublicView, own: PrivateView, id: string): Action | null {
  ...
  if (own.canTrade) {
    const wanted = pub.buildings.some(b => b.playerId === id && b.kind === 'settlement') ? COSTS.city : COSTS.settlement, goods: Good[] = [...RESOURCES, ...COMMODITIES];
    const value = (cards: typeof own.hand) => goods.reduce((n, r) => n + (cards[r] ?? 0) * ((own.hand[r] ?? 0) < (wanted[r] ?? 0) ? 2 : 1), 0);
    const offer = pub.offers.find(o => own.legal.offers.includes(o.id) && !o.accepts.includes(id) && goods.every(r => (own.hand[r] ?? 0) >= (o.get[r] ?? 0)) && goods.some(r => (o.give[r] ?? 0) > 0 && (own.hand[r] ?? 0) < (wanted[r] ?? 0)) && value(o.give) >= value(o.get));
    if (offer) return send({ type: 'accept-offer', offerId: offer.id });
  }
