...
export type Player = RoundContext['players'][number] & { hand: Hand; development: { id: string; kind: DevKind; bought: number }[]; turns: number; played: boolean; freeRoutes: number; moved: boolean; builtShips: string[]; knights: number; longestRoute: number; islands: number[]; connected: boolean; discardDue: number; goldDue: number };
export type State = {
